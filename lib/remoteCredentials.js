...
import {caches, getProfileEdv, profileManager} from './state.js';
import VerifiableCredentialStore from 'bedrock-web-vc-store';

export async function getRemoteVcStore({profileId}) {
  return caches.remoteProfileVcStores.memoize({
    key: 'profile-' + profileId,
    fn: () => _initRemoteVcStore({profileId})
  });
}
...
async function _initRemoteVcStore({profileId}) {
  ...
  return new VerifiableCredentialStore({edv, capability, invocationSigner});
}
