...
function createMetaData({credential, credentials, parentId}) {
  ...
  if(credential.type.includes('OverAgeTokenCredential') && parentId) {
    const issuer = _getAgeVerificationContainerIssuer({credentials});
    meta.parentId = parentId;
    // FIXME: this name is confusing with `bundles` also being a concept,
    // so replace this with some other virtual name
    meta.container = {
...
function _getAgeVerificationContainerIssuer({credentials}) {
...
// FIXME: move me to bedrock-vue-wallet? this does bespoke queries
...
// FIXME: move me to bedrock-vue-wallet? this creates virtual VCs for display
...
export async function addCredential({
  ...
  if(credential.type.includes('AgeVerificationContainerCredential')) {
    ...
    // FIXME: take note that remote store uses displayable `false` for
