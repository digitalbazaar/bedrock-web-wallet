...
export const supportedProofTypes = new Set([
...
export function _checkSignerType({acceptedProofTypes, signer}) {
  const signerType = supportedSignerTypes.get(signer.type);
  if(!acceptedProofTypes.some(({name}) => name === signerType)) {
    throw new Error(
      `"acceptedProofTypes" must contain signer type "${signerType}".`);
  }
}
