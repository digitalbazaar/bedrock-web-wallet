...
// FIXME: update to `@digitalbazaar/edv-client@11`
...
export async function createCapabilities({profileId, request}) {
  ...
  const {
    invocationTarget: target, invoker, delegator,
    referenceId, revocationReferenceId, allowedAction, caveat
  } = request;
  let {parentCapability} = request;
  let revocationZcapRequest;
  ...
  if(
    invocationTarget.type === 'Ed25519VerificationKey2018' ||
    ...
    // delegate revocations zcap if `delegator` was requested
    if(delegator) {
      const keystoreAgent = await profileManager.getProfileKeystoreAgent(
        {profileId});
      if(!keystoreAgent) {
      ...
      revocationZcapRequest = {
        invocationTarget: {
          id: `${keystoreId}/revocations`,
          type: 'urn:webkms:revocations'
        },
        parentCapability: `${keystoreId}/zcaps/revocations`,
        invoker,
        delegator,
        referenceId: revocationReferenceId || `${referenceId}-revocations`,
    ...
    const {edvClient} = await profileManager.createProfileEdv({
      profileId,
      referenceId:
        `${window.location.hostname}:${await EdvClient.generateId()}`,
    });
  ...
  // delegate revocations zcap if `delegator` was requested
  if(revocationZcapRequest) {
    const revocationZcap = await profileManager.delegateCapability({
      profileId,
      request: revocationZcapRequest
    });
    zcaps.push(revocationZcap);
  }

  return zcaps;
}

// async function createKey({type, profileId, profileManager, proofPurpose}) {
//   const keystoreAgent = await profileManager.getProfileKeystoreAgent(
//     {profileId});
//   if(!keystoreAgent) {
//     throw new Error('Unable to find keystore agent for account.');
...
//   return {key, keyDescription, keystoreId: keystoreAgent.keystore.id};
// }
...
export const createProfile = async ({profileAgentContent, profileContent}) => {
  ...
  const {edvClient} = await profileManager.createProfileEdv(
    {profileId, referenceId: config.wallet.DEFAULT_EDVS.users});
  ...
  const {edvClient: credentialsEdvClient} = await profileManager
    .createProfileEdv({
      profileId,
      referenceId: config.wallet.DEFAULT_EDVS.credentials
    });
  ...
  const {zcaps: credentialsEdvZcaps} = await profileManager
    .delegateEdvCapabilities({
      ...
      referenceIdPrefix: config.wallet.DEFAULT_EDVS.credentials,
    });

  // add zcaps to profile agent to use profile's credentials EDV
  for(const zcap of credentialsEdvZcaps) {
    user.zcaps[zcap.referenceId] = zcap;
  }
