...
export class VcapiExchange extends Exchange {
   ...
   * @param {object} options.interaction - The initiating
   ...
   */
  constructor({interaction, exchangeUrl, event} = {}) {
    super({interaction, protocol: 'vcapi'});
  ...
  async next({verifiablePresentation, signOptions} = {}) {
    ...
    if(verifiablePresentation) {
      ...
      if(signOptions) {
        // get `domain` and `challenge` from VPR
        const {
        ...
        const {suite} = signOptions;
        verifiablePresentation = await presentations.sign({
          challenge, domain, presentation: verifiablePresentation, suite
        });
      }
    }
    ...
    // validate any received presentation
    let receivedPresentation = data.verifiablePresentation ?? null;
    if(receivedPresentation) {
      // for backwards compatibility with this library, treat an empty
      // presentation as `null`
      if(Object.keys(receivedPresentation).length === 0) {
        receivedPresentation = null;
      } else {
        validate({type: 'VerifiablePresentation', doc: receivedPresentation});
      }
    }

    // if nothing else is requested or exchange is explicitly marked `done`,
    // then exchange is finished
    if(!data.verifiablePresentationRequest || data.done === true) {
      this._vpr = null;
      this._finish({result: {verifiablePresentation: receivedPresentation}});

      // for backwards compatibility with this library, return specific value
      // when `done`; a major release should just return `data`
      let value = null;
      if(receivedPresentation || data.redirectUrl) {
        value = {};
        if(receivedPresentation) {
          value.verifiablePresentation = receivedPresentation;
        }
        if(data.redirectUrl) {
...
export async function createIfSupported({interaction, event} = {}) {
  ...
  if(exchangeUrl) {
    ...
    return new VcapiExchange({interaction, event, exchangeUrl});
  }
  return null;
}
