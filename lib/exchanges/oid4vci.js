...
export class OID4VCIExchange extends Exchange {
  ...
  async next({signOptions} = {}) {
    ...
    if(this._firstStep) {
      // on first step, so return credential offer
      this._firstStep = false;
      return {value: {credentialOffer: this.offer}, done: false};
    }
    ...
    // do OID4VCI exchange
    const result = await this.client.requestCredentials({did, didProofSigner});

    // normalize result to a VP
    const credentials = result?.credential_responses?.map(r => r.credential);
    const vp = {
    ...
    return this._finish({verifiablePresentation: vp});
  }
}
...
function _getCredentialOffer({event} = {}) {
  ...
  const {web: {protocols}} = event.credentialRequestOptions;
  const url = protocols?.OID4VCI ?? protocols?.OID4VC ?? null;
  if(url) {
    try {
