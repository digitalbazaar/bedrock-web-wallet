...
export class OID4VCIExchange extends Exchange {
  /**
   * Creates a new Exchange that will use the OID4VCI protocol.
   *
   ...
   *
   * @returns {OID4VCIExchange} - A new VcapiExchange instance.
   */
  constructor({interaction, offer, client} = {}) {
    super({interaction, protocol: 'OID4VCI'});
    this.offer = offer;
    this._client = client;
  ...
  async next({verifiablePresentation, signOptions} = {}) {
    ...
    if(signOptions) {
      const {signer, suite} = signOptions;
    ...
    // OID4VP 1.0+ response; will be:
    // {credentials: [credential, ...]}
    let credentials = result.credentials;
    if(!credentials) {
      // OID4VP Draft 13 response; could be any of these:
      // {credential_responses: [{credentials: [{credential}, ...]}, ...]}
      // {credential_responses: [{credentials: [credential, ...]}, ...]}
      // {credential_responses: [{credential}, ...]
      // {credential_responses: [credential, ...]
      credentials = result?.credential_responses ?? (result && [result]);
      credentials = credentials
        ?.map(r => r?.credentials ?? r).flat()?.map(r => r?.credential ?? r);
    }
    ...
    this._finish({result: {verifiablePresentation: vp}});
    return {value: {verifiablePresentation: vp}, done: true};
  }
}

export async function createIfSupported({interaction} = {}) {
  const offer = await _getCredentialOffer({interaction});
  if(offer) {
    try {
      const client = await OID4Client.fromCredentialOffer({offer});
      return new OID4VCIExchange({interaction, offer, client});
    } catch(e) {
...
async function _getCredentialOffer({interaction} = {}) {
