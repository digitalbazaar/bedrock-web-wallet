...
export function normalizeQueryByExample({query} = {}) {
  const {credentialQuery} = query;
  const newCredentialQuery = Array.isArray(credentialQuery) ?
    credentialQuery[0] : credentialQuery ?? {};

  // normalize `acceptedCryptosuites`
  let {acceptedCryptosuites = []} = newCredentialQuery;
  if(!Array.isArray(acceptedCryptosuites)) {
    // invalid `acceptedCryptosuites` value, reset to empty array
    acceptedCryptosuites = [];
  } else {
    acceptedCryptosuites = _normalizeAcceptedCryptosuites(acceptedCryptosuites);
  }
  console.log(
    'Normalized accepted cryptosuites for credentialQuery',
    credentialQuery, acceptedCryptosuites);

  // normalize `acceptedEnvelopes`
  let {acceptedEnvelopes = []} = newCredentialQuery;
  if(!Array.isArray(acceptedEnvelopes)) {
    // invalid `acceptedEnvelopes` value, reset to empty array
    acceptedEnvelopes = [];
  } else {
    acceptedEnvelopes = _normalizeAcceptedEnvelopes(acceptedEnvelopes);
  }
  console.log(
    'Normalized accepted envelopes for credentialQuery',
    credentialQuery, acceptedEnvelopes);

  // normalize `example`
  let {example = {}} = newCredentialQuery;
  if(!(example && typeof example === 'object')) {
    example = {};
  }
  newCredentialQuery.example = example;
...
function _normalizeAcceptedEnvelopes(acceptedEnvelopes) {
  // remove any invalid values; normalize to objects
