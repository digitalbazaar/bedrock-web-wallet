...
export class OID4VPExchange extends Exchange {
  /**
   * Creates a new Exchange that will use the OID4VP protocol.
   *
   ...
   *
   * @returns {OID4VPExchange} - A new VcapiExchange instance.
   */
  constructor({authorizationRequest, verifiablePresentationRequest} = {}) {
    super({protocol: 'OID4VP'});
  ...
  async next({verifiablePresentation, signOptions} = {}) {
    ...
    this._finish({result: value});
    return {value, done: true};
  }
}

export async function createIfSupported({interaction} = {}) {
  const url = await _getAuthorizationRequestUrl({interaction});
  if(url) {
    try {
      ...
      return new OID4VPExchange({
        authorizationRequest, verifiablePresentationRequest
      });
    } catch(e) {
      const args =
...
async function _getAuthorizationRequestUrl({interaction} = {}) {
