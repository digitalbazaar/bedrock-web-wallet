...
export class ChapiDirectExchange extends Exchange {
  /**
   * Creates a new Exchange that will use the CHAPI direct protocol.
   *
  ...
  constructor({interaction, event} = {}) {
    super({interaction, protocol: 'chapi', event});
  ...
  async next({verifiablePresentation, signOptions} = {}) {
    ...
    if(this._event.type === 'credentialstore') {
...
export async function createIfSupported({interaction, event} = {}) {
  ...
  if(event.type === 'credentialstore') {
    try {
      ...
      console.warn(
        'Could not parse Verifiable Presentation for ' +
        'CHAPI direct exchange processing:', e);
      return null;
    }
  }
  ...
  return new ChapiDirectExchange({interaction, event});
}
