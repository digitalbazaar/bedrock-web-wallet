...
import {createCredentialStores} from './credentialStores.js';
...
async function _getCredentialStore({profileId, password}) {
  ...
  // create local and remote VC stores, pairing each store's EDV client with
  // the options from its own config block
  const {local, remote} = createCredentialStores({
    credentialStoreConfig: config.wallet.credentialStore,
