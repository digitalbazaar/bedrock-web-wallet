...
export async function generateQrCodeDataUrl({credential}) {
  ...
  const {imageDataUrl} = await vpqr.toQrCode({
    vp, documentLoader: securityDocumentLoader,
    // use legacy-singleton for backwards compatibility
    format: 'legacy-singleton',
    // use base32 encoding for backwards compatibility
    qrMultibaseEncoding: 'B',
    moduleSize: 3
  });
  return imageDataUrl;
}
...
export async function reissue({
  ...
  while(true) {
    ...
    if(ageVerificationContainerId) {
      credentialStore.delete({id: ageVerificationContainerId}).catch(e => {
...
async function _attemptReissue({
  ...
  // kick off a request to try to get any personal photo VC concurrently as an
  // optimistic guess that it will match the age VC we fetch below
  const photoVcResultsPromise = credentialStore.remote.find({
    query: {type: 'PersonalPhotoCredential'},
    options: {limit: 1}
  });
  // ensure any error isn't thrown; will be processed below
  photoVcResultsPromise.catch(e => e);
  ...
  const ageVcResults = await credentialStore.remote.find({
    query: {type: 'AgeVerificationCredential'},
    options: {limit: 1}
  });
  ...
  // get bundling container VC ID if not previously given -- to ensure it is
  ...
  // see if fetched photo VC is a match
  let photoCredential;
  const guessPhotoVcResults = await photoVcResultsPromise;
  if(!(guessPhotoVcResults instanceof Error) &&
    guessPhotoVcResults.documents.length > 0 &&
    guessPhotoVcResults.documents[0].meta.bundledBy &&
    guessPhotoVcResults.documents[0].meta.bundledBy
      .includes(ageVerificationCredential.id)) {
    ({documents: [{content: photoCredential}]} = guessPhotoVcResults);
  } else {
    // guess not a match; get matching bundled photo credential to ensure it
    // gets bundled with the newly reissued age verification VC
    ...
    if(photoVcResults.documents.length === 0) {
      ...
      await credentialStore.delete({id: remoteContainerId});
      ageVerificationContainerId = null;
      return {success: false, credentials: null, ageVerificationContainerId};
    }

    ({documents: [{content: photoCredential}]} = photoVcResults);
  }
  ...
  const verifiablePresentation = vc.createPresentation({
    ...
    // set `now` to issuance date to avoid any local checks on expiration,
    // letting the server handle it
    now: new Date(ageVerificationCredential.issuanceDate)
  });
