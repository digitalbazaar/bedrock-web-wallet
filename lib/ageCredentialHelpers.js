...
import {getCredentialStore} from './state.js';
...
export async function reissue({ageVerificationContainerId, profileId} = {}) {
...
export async function ensureLocalCredentials({profileId} = {}) {
  const credentialStore = await getCredentialStore({profileId});
