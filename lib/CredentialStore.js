...
export class CredentialStore {
  ...
  async add({
    credentials, records, transformEnvelopedCredentials = false
  } = {}) {
    ...
    // auto-process all enveloped credentials, if requested
    if(transformEnvelopedCredentials) {
      records = await Promise.all(records.map(
        ({credential, meta}) => envelopes.transformEnvelopedCredential({
          credential, meta
        })));
    }

    // ensure all credentials are VCs
    const nonVc = records.find(
      ({credential}) => credential.type?.[0] !== 'VerifiableCredential');
    if(nonVc) {
      console.log('Invalid credential; non-VC', nonVc);
      throw new Error(
        'Invalid credential; all credentials must be expressed as ' +
