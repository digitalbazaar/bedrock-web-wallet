...
export async function renderToNfc({credential} = {}) {
  ...
  // require template/payload field
 ...
 * @returns {Promise<object>} Object with bytes property: {bytes: Uint8Array}.
 */
// export async function renderToNfc_V0({credential} = {}) {
...
function _hasTemplate({renderMethod} = {}) {
  ...
  if(renderMethod.type === 'TemplateRenderMethod') {
    ...
    if(renderMethod && renderMethod.template) {
      return true;
    }
    return false;
  }
  ...
  if(renderMethod.type === 'NfcRenderingTemplate2024') {
    ...
    if(renderMethod && renderMethod.payload) {
      return true;
    }
    return false;
  }
...
function _filterCredential({credential, renderMethod} = {}) {
  ...
  if(!renderProperty || renderProperty.length == 0) {
    return null;
  }

  const filteredData = {};

  // extract each field specified in renderProperty
  for(const pointer of renderProperty) {
    const value = _resolveJSONPointer({obj: credential, pointer});

    // ensure property exists in credential
    if(value === undefined) {
      throw new Error(`Property not found in credential: ${pointer}`);
    }

    // extract field name form pointer for key
    // e.g., "/credentialSubject/name" -> "name"
    const fieldName = pointer.split('/').pop();
 ...
 * @param {object} options - Options object.
 * @param {object} options.renderMethod - The render method object.
 * @returns {boolean} - 'true' if has appropriate field for render method type.
 */
...
//   if(renderMethod.type === 'NfcRenderingTemplate2024') {
//     if(renderMethod.template && renderMethod.payload) {
//       throw new Error('NfcRenderingTemplate2024 should not have' +
//        ' both template and payload fields.'
//       );
//     }
//     // legacy format: ONLY accept 'payload' field
//     encoded = renderMethod.payload;
//     if(!encoded) {
//       throw new Error('NfcRenderingTemplate2024 requires "payload" field.');
//     }
//   } else if(renderMethod.type === 'TemplateRenderMethod') {
//     // W3C Spec format: ONLY accept 'template' field
//     if(renderMethod.template && renderMethod.payload) {
//       throw new Error('TemplateRenderMethod requires "template"' +
//         ' and should not have both fields.'
//       );
//     }
//     encoded = renderMethod.template;
//     if(!encoded) {
...
//     }
//   } else {
//     // This should never happen given _findNFCRenderMethod() logic
//     throw new Error(`Unsupported render method type: ${renderMethod.type}`);
//   }
