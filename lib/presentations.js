...
export async function match({
  ...
  // FIXME: if `group` is specified in the `QueryByExample`, then an `OR`
  // condition is triggered, so initial processing should create groups from
  // the VPR first and then handle those independently
  const and = [];
  const promises = [];
...
export async function sign({
  challenge, domain, presentation, profileId, acceptedCryptosuites,
  ...
  if(!suite && Array.isArray(acceptedCryptosuites) &&
    ...
    for(const {cryptosuite} of acceptedCryptosuites) {
      suite = supportedSuites.get(cryptosuite);
      if(suite) {
        break;
      }
    }
...
async function _deriveCredentials({
  ...
  console.log(
    'Normalized accepted cryptosuites for credentialQuery',
    credentialQuery, acceptedCryptosuites);
...
// this specialized function is to be replaced with more comprehensive
// matching later that is no longer openbadge-specific
function _openBadgeFilter({credentialQuery}) {
  return ({record: {content}}) => {
    const {example} = credentialQuery;
    // get query and content `achievement.id`
    const queryAchievementId = example?.credentialSubject?.achievement?.id;
    const contentAchievementId = content?.credentialSubject?.achievement?.id;
    // match on wildcard only if the content has the property
    const achievementIdMatch = (queryAchievementId === contentAchievementId) ||
      (queryAchievementId === '' && contentAchievementId !== undefined);

    // get query and content `achievement.achievementType`
    const queryAchievementType =
      example?.credentialSubject?.achievement?.achievementType;
    const contentAchievementType =
      content?.credentialSubject?.achievement?.achievementType;
    // match on wildcard only if the content has the property
    const achievementTypeMatch =
      (queryAchievementType === contentAchievementType) ||
      (queryAchievementType === '' && contentAchievementType !== undefined);

    // do not match if a query filter is present but the content doesn't match
    if((queryAchievementId !== undefined && !achievementIdMatch) ||
     (queryAchievementType !== undefined && !achievementTypeMatch)) {
      return false;
    }

    // otherwise, consider the content a match
    return true;
  };
}
