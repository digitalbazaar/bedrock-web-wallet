...
export async function sign({
  ...
  // FIXME: always use `Ed25519Signature2020` if `data-integrity/v1` context is
  // present in any VCs as a backwards compatibility measure
...
async function _deriveCredential({verifiableCredential, proof, pointers}) {
  try {
...
async function _getMatches({vprQuery, credentialStore}) {
  ...
  for(const result of results) {
    ...
    await _deriveCredentials({vprQuery: updatedQuery, matches: result.matches});
  }
