...
import {deriveMdocMdlCredential} from './mdoc.js';
...
export function pruneCredentialProofs({
  ...
  const pruned = vcs
    ...
    .filter(credential => {
      ...
      // `application/mdl-vp-token` is known today
      return credential?.id?.startsWith('data:application/mdl-vp-token');
    });
...
async function _deriveEnvelopedCredential({
  ...
  if(envelope.mediaType === 'application/mdl') {
