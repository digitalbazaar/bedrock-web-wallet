...
export async function createSignOptions({
  ...
  if(authnOption.type === 'Profile') {
    ...
    const {id, signer} = await _importConfidenceMethod({
  ...
  if(authnOption.type === 'ConfidenceMethod' && authnOption.value.id === null) {
    ...
    const algorithm = params.cryptosuite.startsWith('ecdsa') ?
      'P-256' : 'Ed25519';
    params.newConfidenceMethod = await _createConfidenceMethod({
...
async function _createConfidenceMethod({
...
async function _importConfidenceMethod({confidenceMethod}) {
  ...
  if(confidenceMethod.jwk?.crv === 'Ed25519') {
    verificationKeyPair = await Ed25519Multikey.from(confidenceMethod.multikey);
