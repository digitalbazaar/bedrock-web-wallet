...
export async function match({
  ...
  for(const q of query) {
    ...
    promises.push(_matchQueryByExample({query: q, credentialStore, matches}));
  }
...
async function _deriveCredentials({vprQuery, matches}) {
  // FIXME: can assume a single `vprQuery.credentialQuery`

  // FIXME: convert `vprQuery.credentialQuery.example` to JSON pointers

  // FIXME: for each match, if `content` has no understood SD proof, skip it,
  // otherwise generate a derived VC for each understood SD proof, setting
