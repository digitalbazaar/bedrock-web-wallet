...
export async function createSignOptions({
  ...
  if(authnOption.type === 'NewId') {
    ...
    // FIXME: create new signer

    // FIXME: additional params include on device vs. portable key
    // FIXME: use `acceptedCryptosuites` from VPR
    throw new Error('Not implemented');
  }

  // confirm there is a compatible cryptosuite
  const {signer, cryptosuite} = params;
