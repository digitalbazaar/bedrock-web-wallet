...
import {
  createConfidenceMethod, importConfidenceMethod
} from './confidenceMethods.js';
import {deriveMdocMdlCredential, isDeviceResponse} from './mdoc.js';
import {
...
// key by `signer.type` or `signer.algorithm`
// `signer.algorithm` can be a signing algorithm or an ECC curve
...
const SUPPORTED_ENVELOPE_MEDIA_TYPES = new Set([
  'application/mdoc',
  // deprecated
  'application/mdl'
]);
...
export async function createSignOptions({
  ...
  console.log('Creating sign options for authentication option id+type', {
    id: authnOption.value.id,
    type: authnOption.value.type
  });
  ...
  if(authnOption.type === 'Profile') {
    ...
    params.controller = profileId;
  } else if(authnOption.type === 'ConfidenceMethod' &&
    authnOption.value.id !== null) {
    const {id, signer} = await importConfidenceMethod({
      confidenceMethod: authnOption.value
    });
    params.signer = signer;
    params.controller = id;
  }

  // get `acceptedCryptosuites` from VPR
  const {acceptedCryptosuites} = await getAcceptedCryptosuites({
    verifiablePresentationRequest,
    // FIXME: for backwards compatibility only, remove `signer` param in future
    signer: params.signer
  });

  // pick a compatible cryptosuite
  ...
  // create a new confidence method, if requested, using chosen cryptosuite
  if(authnOption.type === 'ConfidenceMethod' && authnOption.value.id === null) {
    // FIXME: additional params include on-device, portable key, or WebKMS key
    const algorithm = params.cryptosuite.startsWith('ecdsa') ?
      'P-256' : 'Ed25519';
    params.newConfidenceMethod = await createConfidenceMethod({
      didMethod: 'key', algorithm
    });
    params.signer = params.newConfidenceMethod.signer;
    params.controller = params.newConfidenceMethod.id;
    console.log('Created new confidence method', {
      id: params.newConfidenceMethod.id, algorithm
    });
  }

  // return sign options
  const {signer, controller, cryptosuite, newConfidenceMethod} = params;
  return {
    profileId,
    signer,
    controller,
...
export async function getCompatibleAuthnOptions({
  verifiablePresentationRequest, profileId, confidenceMethods
} = {}) {
  const {acceptedCryptosuites} = await getAcceptedCryptosuites({
    verifiablePresentationRequest,
    // for backwards compatibility, allow `acceptedCryptosuites` to be
    // generated from `profileId` and `confidenceMethods`
    profileId, confidenceMethods
  });

  // generate viable authn options...
  const authnOptions = [];

  // add any viable profile authn option
  if(profileId) {
    const {
  ...
  // add any viable confidence method authn options
  if(confidenceMethods) {
    const accepted = new Set(acceptedCryptosuites.map(ac => ac?.cryptosuite));
    for(const confidenceMethod of confidenceMethods) {
      const supported = SUPPORTED_CRYPTOSUITES.get(confidenceMethod.jwk?.crv);
      if(supported.intersection(accepted).size > 0) {
        authnOptions.push({
  ...
  // if any accepted cryptosuite is compatible with a supported suite, add
  // an authn option to create a new confidence method
  for(const {cryptosuite} of acceptedCryptosuites) {
    if(supportedSuites.has(cryptosuite)) {
      authnOptions.push({
        type: 'ConfidenceMethod',
        value: {id: null, type: 'DecentralizedIdentifierDocument'}
      });
      break;
    }
  }
...
export async function getAcceptedCryptosuites({
  verifiablePresentationRequest,
  // for backwards compatibility, allow `acceptedCryptosuites` to be generated
  // from `signer`, `profileId`, and `confidenceMethods`
  signer, profileId, confidenceMethods
} = {}) {
  // FIXME: process all possible DIDAuthentication queries, not just one
  ...
  console.log('DIDAuthentication options', didAuthnOptions);

  // FIXME: if `acceptedCryptosuites` is empty, return empty set, but only in
  // a major revision as backwards compatibility in this library (erroneously)
  // requires setting default `acceptedCryptosuites`
  let {acceptedCryptosuites} = didAuthnOptions;
  if(!(acceptedCryptosuites?.length > 0)) {
    // add all possible supported suites based on `signer`, `profileId`,
    // and `confidenceMethods`
    const supported = new Set();
    if(signer) {
      const toAdd = SUPPORTED_CRYPTOSUITES.get(signer?.type) ??
        SUPPORTED_CRYPTOSUITES.get(signer?.algorithm);
      toAdd.forEach(c => supported.add(c));
    }
    if(profileId) {
      const {
        invocationSigner: profileSigner
      } = await profileManager.getProfileSigner({profileId});
      const toAdd = SUPPORTED_CRYPTOSUITES.get(profileSigner?.type) ??
        SUPPORTED_CRYPTOSUITES.get(profileSigner?.algorithm);
      toAdd.forEach(c => supported.add(c));
    }
    if(confidenceMethods) {
      for(const confidenceMethod of confidenceMethods) {
        const toAdd = SUPPORTED_CRYPTOSUITES.get(confidenceMethod.jwk?.crv);
        toAdd.forEach(c => supported.add(c));
      }
    }
    if(supported.size === 0) {
      // default to `P-256` signatures for widest interop
      const toAdd = SUPPORTED_CRYPTOSUITES.get('P-256');
      toAdd.forEach(c => supported.add(c));
    }
    // return all supported suites as acceptable
...
export async function match({
  requestOrigin, verifiablePresentationRequest, credentialStore
} = {}) {
...
export function pruneCredentialProofs({
  ...
  const pruned = vcs
    .map(credential => _pruneCredentialProofs({credential, proofAllowList}))
    .filter(credential => {
      // FIXME: improve code to ensure this hack to protect against leaking
      // enveloped VCs can be removed; other code should ensure that no
      // enveloped VC that cannot be shared should reach this
      const isEnveloped =
        (credential?.type === 'EnvelopedVerifiableCredential' ||
        credential?.type?.[0] === 'EnvelopedVerifiableCredential');
      if(!isEnveloped) {
        return true;
      }
      // filter out any unknown envelope media types for presentation; only
      // mdoc device responses are supported today
      return isDeviceResponse({credential});
    });
...
async function _deriveCredentials({
  requestOrigin, verifiablePresentationRequest, vprQuery, matches
}) {
  const {credentialQuery} = vprQuery;
  const {acceptedCryptosuites, acceptedEnvelopes} = credentialQuery;
  if(acceptedCryptosuites.length === 0 &&
    acceptedEnvelopes.length === 0) {
    // verifier does not mention any accepted SD cryptosuites nor envelopes,
    // so nothing to do
    return;
  }
  ...
  // add any derived VCs using data integrity...

  // for each match, if `content` has no understood SD proof, skip it,
  // otherwise generate a derived VC for each SD proof that is both understood
  // by the wallet and in `credentialQuery.acceptedCryptosuites`, pushing
  // `{cryptosuite: <cryptosuite>, derivedCredential}` to `match.derived`
  ...
  // add any derived VCs using envelopes...
  // for each match, if credential has an envelope with a media type that is
  // both supported and in `credentialQuery.acceptedEnvelopes` create a
  // derived credential and push
  // `{envelope: <mediaType>, derivedCredential}` to `match.derived`
  // FIXME: improve matching for envelope elements with:
  // `{mediaType, meta: {docType}}`
  const acceptedEnvelopeSet = new Set(acceptedEnvelopes.map(
    ae => ae?.mediaType));
  for(const match of matches) {
    const {
      record: {content: verifiableCredential, meta}
    } = match;
    if(!match.derived) {
      match.derived = [];
    }

    // if no supported envelope, cannot generate derived VC
    const {envelope} = meta;
    if(!(SUPPORTED_ENVELOPE_MEDIA_TYPES.has(envelope?.mediaType) &&
       envelope?.credential && acceptedEnvelopeSet.has(envelope?.mediaType))) {
      continue;
    }

    const derived = await _deriveEnvelopedCredential({
      requestOrigin, verifiablePresentationRequest,
      verifiableCredential, envelope, meta, pointers
    });
    if(derived) {
      console.log('Derived enveloped credential', envelope, derived);
    }
    match.derived.push(derived);
  }
...
async function _deriveEnvelopedCredential({
  requestOrigin, verifiablePresentationRequest,
  verifiableCredential, envelope, meta, pointers
} = {}) {
  // `mdoc` preferred; `mdl` deprecated
  if(envelope.mediaType === 'application/mdoc' ||
    envelope.mediaType === 'application/mdl') {
    return deriveMdocMdlCredential({
      requestOrigin, verifiablePresentationRequest,
      verifiableCredential, envelope, meta, pointers
    });
  }
...
async function _getMatches({
  requestOrigin, verifiablePresentationRequest, vprQuery, credentialStore
}) {
...
async function _matchQueryByExample({
  ...
  matches.push(...await _getMatches({
    requestOrigin, verifiablePresentationRequest,
    vprQuery: query, credentialStore
  }));
}
