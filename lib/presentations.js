...
export async function createSignOptions({
  ...
  const {acceptedCryptosuites} = await getAcceptedCryptosuites({
    verifiablePresentationRequest,
    // FIXME: for backwards compatibilty only, remove `signer` param in future
...
export async function getAcceptedCryptosuites({
  ...
  if(!(acceptedCryptosuites?.length > 0)) {
    // determine all signer supported suites
    let signerSupported;
    if(signer) {
      signerSupported = SUPPORTED_CRYPTOSUITES.get(signer?.type) ??
        SUPPORTED_CRYPTOSUITES.get(signer?.algorithm);
    }
    if(!signerSupported) {
      // default to `P-256` signatures for widest interop
      signerSupported = SUPPORTED_CRYPTOSUITES.get('P-256');
    }
    // return all signer supported suites as acceptable
    acceptedCryptosuites = [...signerSupported].map(
