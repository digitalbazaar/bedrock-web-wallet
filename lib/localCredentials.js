...
export async function getLocalVcStore({profileId}) {
  const localProfileVcStoreCache = await defaultStore.get({
    id: 'localProfileVcStoreCache'
  });
  return localProfileVcStoreCache.memoize({
    key: 'profile-' + profileId,
    fn: () => initLocalVcStore({profileId})
  });
}

export async function initLocalVcStore({profileId}) {
