...
export async function importConfidenceMethod({confidenceMethod} = {}) {
  ...
  let publicJwk;
  if(algorithm.startsWith('P-')) {
    publicJwk = await EcdsaMultikey.toJwk({
      keyPair: verificationKeyPair,
      secretKey: true
    });
  } else {
    publicJwk = await Ed25519Multikey.toJwk({
      keyPair: verificationKeyPair,
      secretKey: true
    });
  }
  ...
  return {
    id: verificationKeyPair.controller, type, signer,
    privateJwk: structuredClone(confidenceMethod.jwk),
    publicJwk
  };
}
